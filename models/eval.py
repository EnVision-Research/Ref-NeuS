...
def evaluation(data, scan, dataset_dir, vis_out_dir, mode='mesh', downsample_density=0.2, patch_size=60, max_dist=20, visualize_threshold=10):
    mp.freeze_support()
    ...
    stl_pcd = o3d.io.read_point_cloud(f'{dataset_dir}/Points/stl/stl{scan:03}_total.ply')
    ...
    write_vis_pcd(f'{vis_out_dir}/vis_{scan:03}_d2s.ply', data_down, data_color)
    stl_color = np.tile(B, (stl.shape[0], 1))
    stl_alpha = dist_s2d.clip(max=vis_dist) / vis_dist
    stl_color[ np.where(above)[0] ] = R * stl_alpha + W * (1-stl_alpha)
    stl_color[ np.where(above)[0][dist_s2d[:,0] >= max_dist] ] = G
    write_vis_pcd(f'{vis_out_dir}/vis_{scan:03}_s2d.ply', stl, stl_color)
...
def evaluation_shinyblender(data, dataset_dir, vis_out_dir, downsample_density=0.3, patch_size=60, max_dist_d=100, 
